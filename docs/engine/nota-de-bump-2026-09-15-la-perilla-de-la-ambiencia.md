...
3. **Es un ajuste del instrumento, no del font**: sobrevive a `reset()`, a cargar o descargar un font
   ...
   dos atómicos que se leen una vez por bloque — sin cola, sin lock, sin allocation, sin costo nuevo.
...
- **El escalón 0/0 → 1/1 con una nota sostenida** (cambiar el toggle en caliente) se midió antes de
  decidir si llevaba rampa: la derivada pico del wet en el bloque de conmutación es **0,91×** la del
  wet en régimen — no es más brusco que la señal que ya sonaba. **Sin rampa**; el test queda de guarda
  con el número. Si en el device se oye un pop al conmutar, ese es el dato que el número no vio.
