...
- **Hecha: transformaciones DSP puras del loop → `usb/DspBlockOps.h`** (`monoToStereo` −3dB, `spliceDeclickHead`, `fadeBlockToSilence`). El de-click de splice estaba **duplicado** inline (splice-pending + underrun-fade); ahora es una sola función. `test_dsp_block_ops.cpp` con golden values calculados a mano, incluida la secuencia compuesta del underrun (hold→fade→declick) — cubre H6. **Suite 486/486** (479+7). Bit-idéntico (misma matemática/orden; underrun captura hold ANTES del fade y de-clickea el bloque faded). `LibusbBackend.cpp` −23 líneas netas. **Pendiente:** build NDK (no validable acá) — verificar timing con profiler stats antes/después on-device.
- **Costuras siguientes (sin hacer):** `usb/DspLoop` (cuerpo del loop consumiendo DspPacer+DspBlockOps — la más delicada, última), `usb/StreamNegotiator` (configureSampleRate ~817-1060), `usb/UsbVolumeController` (feature units — ojo: ya existe `usb/UsbVolumeControl.h`, revisar qué cubre), `usb/PacketCodec` (solo si se aísla la conversión pura). Cada una: PR propio, bit-idéntico, suite host.
