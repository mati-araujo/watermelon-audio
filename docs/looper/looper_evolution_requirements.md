...
| WL-4.1.c | **Contrato de `stretchTrack` para la performance visual** (carta de NoisyPad del 2026-09-16, decisión del 16/09; sin código hasta que WL-4.1 se abra) | (1) El motor **devuelve lo que aplicó**: `{ratioEffective, newLengthFrames, newLoopStart, newLoopEnd, offsetFrames}`, no sólo `ok` — si el algoritmo redondea a bloques o ajusta el ratio para cerrar en barra, el consumidor no lo re-deriva (precedente: `armInFrames`/`armSyncedToLoop` devuelven el frame absoluto, WV-4.1). (2) **El origen del buffer no se mueve**: `frame 0` antes = `frame 0` después; la latencia/pre-roll del algoritmo se absorbe adentro o se reporta como `offsetFrames` — nunca un corrimiento constante silencioso. La región se preserva proporcionalmente. (3) **El versionado lleva causa**: el `contentVersion` cambia y el consumidor puede distinguir *stretch* (transforma su performance por `ratioEffective`) de *reemplazo* (la descarta); un bump sin causa ya costó con `importTrack`. `rotateTrack(track, frames)`, si entra, con el mismo trío y rotación modular sobre la **región**, no sobre el buffer. C API: `wma_looper_stretch_track` devuelve la estructura o la expone por getters | Toma XY de 4 s → `stretchTrack(1,25)`: `newLengthFrames = round(len × 1,25)` ± 1 bloque; los onsets de `detectOnsets` pre y post, escalados por `ratioEffective`, coinciden ± 1 hop; corrimiento constante ≤ 10 ms y sin crecimiento por vuelta. Control negativo: `ratio = 1,0` deja buffer, región y versión-causa byte a byte. Se hereda como delta declarado del REQ que abra WL-4.1 | P1 | — |
